...
def welcome(*, invocation: str, python: str, key_set: bool, db_path: str, db_exists: bool) -> str:
    ...
    return "\n".join([
        "who-finder — find people, then see who is worth buying.",
        "",
        "Three commands. That is the whole tool.",
        "",
        f'  {invocation} find "CMO AI video ads"',
        f"  {invocation} rate names.csv",
        f'  {invocation} run "CMO AI video ads"',
        "",
        "find   search for people or companies by keywords",
        "rate   score a CSV, a Clay export, or the file find wrote",
        "run    find + rate in one go, writes a report you can forward",
        "",
        "------------------------------------------------------------------------",
        "Just cloned this? Two steps:",
        "",
        f"  1. Python    {python}  — already on a Mac.",
        *key_lines,
        "",
        "If this is in Cursor, you just ask:",
        '    "Find me 10 people making AI video ads."',
        "",
        "Try it now (no key needed):",
        f"    {invocation} doctor",
        f'    {invocation} find "CMO AI video ads"',
        f"    {invocation} rate tests/fixtures/creators.csv",
        f'    {invocation} find "CMO AI video ads" --dry-run',
        "",
        "Guides:  docs/start.md   docs/ask.md",
        "",
